...
foreach($path in @("$project\manage.ps1","$project\tools\autostart.ps1","$project\tools\diagnostics.ps1")) {
...
function Target { [pscustomobject]@{InstanceId='test-gpu';Status=$(if($script:scenario -eq 'device-failure'){'Error'}else{'OK'})} }
...
foreach($script:scenario in 'success','blocked','same-boot','bind-failure','device-failure','memory-failure','diagnostic-failure','handover-failure','restore-failure') {
    ...
    @{InstanceId='test-gpu';OriginalInf="$runtime\backup\original.inf"} | ConvertTo-Json | Set-Content "$runtime\backup\restore.json"
    ...
    & $run
    $expected=@{'success'=6;'blocked'=0;'same-boot'=0;'bind-failure'=1;'device-failure'=1;'memory-failure'=3;'diagnostic-failure'=4;'handover-failure'=5;'restore-failure'=6}[$script:scenario]
    ...
    if($script:scenario -in 'success','same-boot'){if($blocked){throw 'Unexpected failure latch'}}
